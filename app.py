...
def main() -> None:
    # Local: puede leer .env.
    # GitHub Actions: usa Secrets/Variables del workflow, no subas .env al repo.
    load_dotenv_local()

    run_once = env_bool("RUN_ONCE", False)
    send_welcome_on_start = env_bool("SEND_WELCOME_ON_START", default=not run_once)
    ...
    try:
        ...
        if send_welcome_on_start:
            bot.send_welcome()

        # En GitHub Actions conviene ejecutar un solo ciclo y terminar.
        # El cron del workflow se encarga de volver a ejecutar cada 10 minutos.
        bot.run_cycle()

        if run_once:
            logger.info("RUN_ONCE=true: ciclo único terminado. Saliendo.")
            return
